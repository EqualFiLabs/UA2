...
use ua2_contracts::ua2_account::UA2Account::{
    Event, GuardianAdded, GuardianRemoved, RecoveryDelaySet, ThresholdSet,
};
...
fn call_with_guardian(
    contract_address: ContractAddress, selector: felt252, guardian: ContractAddress,
...
fn guardians_admin_works() {
    ...
    call_with_guardian(contract_address, starknet::selector!("add_guardian"), g1).unwrap_syscall();
    call_with_guardian(contract_address, starknet::selector!("add_guardian"), g2).unwrap_syscall();
    call_with_guardian(contract_address, starknet::selector!("add_guardian"), g3).unwrap_syscall();

    let duplicate = call_with_guardian(contract_address, starknet::selector!("add_guardian"), g1);
    assert_reverted_with(duplicate, ERR_GUARDIAN_EXISTS);

    call_with_value(contract_address, starknet::selector!("set_guardian_threshold"), 2.into())
        .unwrap_syscall();
    call_with_value(contract_address, starknet::selector!("set_recovery_delay"), 60_u64.into())
        .unwrap_syscall();
    call_with_guardian(contract_address, starknet::selector!("remove_guardian"), g3)
        .unwrap_syscall();

    let bad_remove = call_with_guardian(
        contract_address, starknet::selector!("remove_guardian"), g4,
    );
    assert_reverted_with(bad_remove, ERR_NOT_GUARDIAN);

    let bad_threshold = call_with_value(
        contract_address, starknet::selector!("set_guardian_threshold"), 3.into(),
    );
    ...
    spy
        .assert_emitted(
            @array![
                ...
                (contract_address, Event::RecoveryDelaySet(RecoveryDelaySet { delay: 60_u64 })),
                (contract_address, Event::GuardianRemoved(GuardianRemoved { addr: g3 })),
            ],
        );
}

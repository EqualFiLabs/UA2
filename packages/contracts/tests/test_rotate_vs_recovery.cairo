...
fn call_without_args(
    contract_address: ContractAddress, selector: felt252,
) -> SyscallResult<Span<felt252>> {
    let empty = array![];
...
fn rotate_requires_no_active_recovery() {
    ...
    call_with_felt(contract_address, starknet::selector!("set_guardian_threshold"), 1_u8.into())
        .unwrap_syscall();
    ...
    call_with_felt(contract_address, starknet::selector!("propose_recovery"), NEW_RECOVERY_OWNER)
        .unwrap_syscall();
    stop_cheat_caller_address(contract_address);

    start_cheat_caller_address(contract_address, contract_address);
    let rotate_during_recovery = call_with_felt(
        contract_address, starknet::selector!("rotate_owner"), ROTATED_OWNER,
    );
    assert_reverted_with(rotate_during_recovery, ERR_RECOVERY_IN_PROGRESS);
    stop_cheat_caller_address(contract_address);

    start_cheat_caller_address(contract_address, contract_address);
    call_without_args(contract_address, starknet::selector!("cancel_recovery")).unwrap_syscall();
    call_with_felt(contract_address, starknet::selector!("rotate_owner"), ROTATED_OWNER)
        .unwrap_syscall();
    stop_cheat_caller_address(contract_address);

...
export class NoopPaymaster implements Paymaster {
  ...
  private readonly sponsorData: string[] | undefined;
  ...
  constructor(options: NoopPaymasterOptions = {}) {
    ...
    this.feeCeiling = options.feeCeiling !== undefined
      ...
      : DEFAULT_FEE_CEILING;
    this.sponsorData = options.sponsorData?.map((felt, idx) => {
      const label = `sponsorData[${idx}]`;
      const value = toBigInt(felt, label);
  ...
  async sponsor(tx: AccountTransaction): Promise<SponsoredTransaction> {
    ...
    if (this.feeCeiling !== undefined) {
      if (maxFee === undefined) {
        ...
        maxFee = formatFelt(parsed);
      }
    }

    return {
      ...tx,
      maxFee: maxFee === undefined ? undefined : (typeof maxFee === 'string' ? formatFelt(toBigInt(maxFee, 'maxFee')) : formatFelt(maxFee)),
